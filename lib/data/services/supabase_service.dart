...
class SupabaseService {
  ...
  // Akses client langsung dari Supabase instance
  ...
  Future<MemberSaas> registerOwner({
    ...
    try {
      ...
      final response = await client.from('member_saas').insert({
        'wa_toko': waToko,
        'nama_toko': namaToko,
  ...
  Future<void> updateMember(String waToko, Map<String, dynamic> data) async {
    try {
      await client.from('member_saas').update(data).eq('wa_toko', waToko);
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<UserKasir> createKasir({
    ...
    try {
      final response = await client.from('user_kasir').insert({
        'wa_owner': waOwner,
        'username': username,
        'password': password,
        'role': 'Kasir',
